...
Round Robin - fixed time slice - if not done in time, move to back
 ...
 - Dis: No job priority - longer jobs take much longer
...
Source code - object code before compiled

**Open Source:**
- available to anyone - no license - distributed with source code
Advantages:
- modified, improved by community/people
- technical support from online community
- can be modified and sold for profit
Disadvantages:
- Support - No official support or user manual
  - support may be outdated, insufficient, incorrect
- Security - Lower security - not developed in controlled environment

**Closed Source:**
- requires license - can't access source code as company owns copyright license
Advantages:
- Updates - Frequent, well-tested updates
- Support - Official expert support + user manuals
- Security - High security - professional developed
Disadvantages:
- License - restricts amount of people using software at once - have to pay
- Cant improve software themselves
...
4) Optimisation - searches to make code more efficient 
      ...
      - Redundancy reduced - redundant code parts removed, grouped code replaced 
      - makes compilation longer - very time-consuming
...
Loader - program provided by OS
  - retrieves library/routine from given memory location 
  - copies program into main memory (RAM) ready for execution
...
constructor Method:
  public procedure new(attribute1, attribute2):
   { 
    actual code 
   }
...
- Procedural programming languages - programming paradigms
- addressing memory

- Databases - 1NF, 2NF, 3NF
...
Pass by value - copy of value is passed to subroutine (different memory address), treated as local variable
...
1) easy to understand, debug, maintain --- understand as a unit of code
...
Advantages:
- functions easier to understand/read - less code lines
- debugging easier than long iterative algorithms (tree+graph traversal, searching, sorting) 
...
Call stack - each time routine called, stack frame added
  - popped from stack at end of routine
stack frame - stores parameters, local variables, return address of that routine
Stack overflow - call stack runs out of memory 
  - recursion infinite = too many stack frames = call stack full 
...
Encapsulation - attributes = private - can only be altered by public methods 
- wraps attributes & methods inside class definition
- attributes cant be directly edited
- for information hiding - program less complex - protects certain data from being accidentally edited by other parts of program
  - top-down has same principle of encapsulation - each module self-contained

#### OOP vs procedural:
OOP:
- objects dont share data
- systems built around objects
- Objects = real-world representations 
Procedural:
- data shared by passing data through or global variables
- system built - combining procedures

OOP Coding (Psuedocode):

CLASS (Class_name)

    PRIVATE (attribute1): String/integer/real (data type)
    PRIVATE (attribute2): (data type)


    PUBLIC PROCEDURE constructor_Name(parameter1, parameter2)
          attribute1 = ... (parameter1)
          attribute2 = ... (parameter2)

    ENDPROCEDURE

    PUBLIC PROCEDURE method_Name()
      // code for method (like a function)

    ENDPROCEDURE

ENDCLASS

- Attributes have to be declared as private (due to **encapsulation**)
- Constructor name usually same as class name in psuedocode
  - or constructor = New, in python = __init__
- value inside constructor brackets = parameters for variables inputted by user



## Computational Methods
#### Process of problem-solving:
Computable problem - can be solved using an algorithm (can be solved with computational methods/a computer)
- computable problems = solveable in a finite, realistic timeframe
- computable problems can be impractical to solve due to processing power, speed, memory, time

Stakeholders state result from solution
- info used to clearly define problem & system requirements
Requirements defined by:
- stengths + weaknesses of current solution
- inputs, outputs, stored data, amount of data

Decomposition - broken down into smaller problems until each sub-problem = self-contained subroutine
- reduces problem complexity
- programmers find sections that can be implemented with libraries/modules - saves time coding
- project easier management - teams assigned modules based on skills
- develop modules in parallel - project done faster
- debugging easier - quicker to identify, location and mitigate errors

Divide-and-conquer:
3 parts: 
1) Divide - halves problem size each iteration
2) Conquer - each subproblem solved recursively
...
levels of abstraction - complex problem divided to smaller parts
  - project more manageable - levels assigned to teams - other layers hidden from each other
abstraction by generalisation - group sections with similar functionality - segments reused - saves time
